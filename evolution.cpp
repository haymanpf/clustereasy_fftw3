...
float gradient_energy(int fld)
{
  ...
  double gradient=0., result;
  float norm=1./pw2(dx); // Converts the output of lapl() to an actual Laplacian

  if(!buffers_up_to_date) update_buffers(); // Update buffers if needed
  ...
  // Iterate over the (j and k) boundaries
#if NDIMS==1 // There are no boundaries for this case
#elif NDIMS==2
  for(i=1;i<=n;i++) // Index in x direction
  {
  ...
  MPI_Allreduce(&gradient, &result, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); // Average gradients from all processors to get the average gradient for the array

  // norm converts the results of lapl() to an actual laplacian and gridsize converts the sum over the lattice to an average
  return(.5*(float)result*norm/(float)gridsize);
}
