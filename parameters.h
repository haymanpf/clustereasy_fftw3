...
// ---Adjustable parameters for T-model--- //
...
const int noutput_times=20; // Number of times at which to calculate and save output variables
...
const float checkpoint_interval=5.; // How often to output a grid image and perform infrequent calculations (see list below). Only done at end if checkpoint_interval=0.
...
const int sspectra=0; // Output power spectra
...
const int sslices=1; // Output the field values on a slice through the lattice
  ...
  const int slicedim=NDIMS; // Dimensions of slice to be output. (If slicedim>=NDIMS the whole lattice will be output.) Warning: If slicedim=3 the resulting file may be very large.
