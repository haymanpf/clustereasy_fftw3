...
const float dt = .05; // Size of time step
const float tf=200.; // Final time
...
const int sslices=1; // Output the field values on a slice through the lattice
  ...
  const int sliceaverage=1; // If sliceskip>1 and sliceaverage=1 the slices will contain averages over all the field values in between sampled points.  Otherwise they will just use the sampled values. This option will be ignored in the parallel version if more than one processor is being used to output slices (which can only occur if slicedim=NDIMS)
